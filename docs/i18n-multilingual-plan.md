...
>
> **Follow-up:** since Cypress genuinely can't run in that sandbox, `e2e/` gained a second, actually-executable suite using `@playwright/test` (`yarn e2e:playwright`), covering the same three specs 1:1. It runs against a production build rather than `next dev` — the dev server's on-demand route compilation shows a floating build-activity indicator (`<nextjs-portal>`) that intercepted clicks mid-navigation and caused reproducible flakiness on the locale-switch test; building once first (matching the existing `lighthouse:ci` convention) eliminated it. All 13 tests pass, including reruns with `--repeat-each` to confirm the flake is actually fixed and not just quieter.
